...
async function runMasterPipelineTest() {
    ...
    try {
        ...
        const userLevel = 12;
        const progression = await calculateProgression(normalized.title, normalized.description, normalized.attribute, userLevel);
        ...
        const quest = await generateAdaptiveQuest(domainContext, normalized.attribute, userLevel);
