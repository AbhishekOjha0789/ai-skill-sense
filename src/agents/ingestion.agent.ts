...
// Define the expected output telemetry interface
export interface TelemetryPayload {
  title: string;
  description: string;
  attribute: 'COGNITIVE' | 'PHYSICAL' | 'EMOTIONAL' | 'TECHNICAL' | 'CREATIVE' | 'FINANCIAL';
  sourceType: string;
}
 ...
 */
export async function normalizeIngestionData(
  rawInput: string, 
  sourceType: string
...
`;
  ...
  try {
    ...
    const telemetry: TelemetryPayload = JSON.parse(content);
    return telemetry;
